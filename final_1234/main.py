...
def recommended():
    # Example: take last scanned product category
    scanned_products = session.get('scanned_products', [])
    last_product_name = scanned_products[-1]['name'] if scanned_products else None
...
def add_recommended():
    ...
    products = session.get('scanned_products', [])
    products.append({"name": product_name, "price": product_price, "quantity": 1})
    session['scanned_products'] = products
...
def get_stock():
    ...
    try:
        ...
        # Note: 'barcodedata' field name assumed based on user's query
        cursor.execute("SELECT barcodedata, product_name, product_price, image, quantity FROM products")
...
def add_product():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO products (barcodedata, product_name, product_price, image, quantity)
            VALUES (%s, %s, %s, %s, %s)
        """, (barcode, name, price, image_url, 0))
        conn.commit()
        return jsonify({'status': 'Product added successfully'})
    except mysql.connector.Error as e:
        print("MySQL Error:", e)
        return jsonify({'status': f'Database error: {e}'}), 500
    except Exception as e:
...
def remove_product_api():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM products WHERE barcodedata = %s", (barcode,))
        conn.commit()
...
# FIXED: Renamed to 'register' to match the endpoint expected by url_for('register') in HTML templates.
    ...
    if request.method == 'POST':
        ...
        if not re.match(r'[^@]+@[^@]+\.[^@]+', email):
            ...
            if account:
                ...
                # The user's original table structure didn't include `password` and `email` for the scanner flow, 
                # so we assume this is the web user table insertion.
...
def login():
    ...
    if request.method == 'POST':
        ...
        if account and check_password_hash(account['password'], password): # Assuming 'password' is the column name
...
# FIXED: Changed route path from '/register' to '/register-scanner' to avoid conflict with the web registration GET route.
...
def register_scanner_user():
    ...
    try:
        # Assuming a 'scanner_users' table or the 'users' table has these fields
        # Using ON DUPLICATE KEY UPDATE ensures this works for existing users, assuming 'phone_number' is unique
        cursor_conn.execute("""
        ...
        # Get the user ID
        cursor_conn.execute("SELECT id FROM users WHERE phone_number = %s", (phone_number,))
        user_id = cursor_conn.fetchone()[0]

        return jsonify({"status": "User registered", "user_id": user_id})
    except mysql.connector.Error as e:
        print(f"MySQL Error in register_scanner_user: {e}")
...
def get_user_details():
    ...
    try:
        cursor_conn.execute("SELECT phone_number, name, address FROM users WHERE id = %s", (user_id,))
        user_details = cursor_conn.fetchone()
...
def barcode_scanner(user_id, delay):
    ...
    # Initialization is now handled by the calling route (start/add-more)
    # The initialization block below is removed to prevent accidental resets
    # 
    # lock.acquire()
    # try:
    #     products = [] 
    ...
    try:
        while scanning:
            ...
            for barcode in barcodes:
                ...
                if result:
                    product_name, product_price_decimal = result
                    product_price = float(product_price_decimal) # FIXED: Convert Decimal to float for addition
...
if __name__ == '__main__':
    ...
    print("\n--- Starting Flask App ---")
    print("WARNING: Please ensure all external dependencies (OpenCV, Pygame, MySQL, sound files) are correctly installed and configured.")
